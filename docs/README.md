...
> 状态：M2（规则与本地化）进行中；M1 只读评审闭环已全部落地（领域类型、forge 接口 + 锚定器、`trust-policy`、`forge-github`、`tool-review`、`review-runtime` 八阶段管线、`progress`、`driver-action`，322 例单测），M2 已交付 `rule-registry` / `rules-baseline` / `forge-local`，`driver-cli` 与 M3/M4 尚未实现
