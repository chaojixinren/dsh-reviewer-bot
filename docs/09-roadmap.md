...
未实现（刻意）：M5 发布与收尾仍在收口——npm 发布（#54）的发布管线已就绪但尚未实际 `npm publish`，`dsh plugin add @dshrb/bundle` 的线上验证待发布后执行。
