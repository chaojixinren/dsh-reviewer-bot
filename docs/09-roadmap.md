...
未实现（刻意）：`forge-github` 的 `commitPatches` / `openPullRequest` 仍是桩（GitHub 侧真实 commit / PR 创建未接，`forge-local` 与 `forge-gitlab` 已实现）；M4 的 跨 PR 记忆 尚未实现。下一步：跨 PR 记忆（后续拆新 issue）。
