...
M5 发布与收尾（进行中）：npm 发布（#54）的发布管线已就绪——`scripts/gen-package-manifests.mjs` 将 bundle 依赖闭包（`review-core` 与九个插件）标记为可发布（去掉 `private`，补 `license` / `repository` / `publishConfig`），新增 `.github/workflows/publish.yml` 按拓扑序构建并发布；`workspace:*` / `workspace:0.1.0` 由 pnpm 在发布时改写为精确 `0.1.0`。LICENSE（#53）已开 PR #55；剩余 runtime bootstrap + release build（#50）、`forge-github` 写能力（#51），以及 #54 的实际 `npm publish`（待合并后手动触发工作流，需 `NPM_TOKEN`）。
